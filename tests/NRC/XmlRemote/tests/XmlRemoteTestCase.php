 ...
 * @package XmlRemote
 */
abstract class XmlRemoteTestCase extends \PHPUnit_Framework_TestCase {

	public function getApi($class, $type) {
		return $this->_instance("\\NRC\XmlRemote\\Api\\{$class}", array(
			'client' => $this->getMockClient($class, $type)
		));
	}

	public function getResponse($class, $type) {
		return $this->_instance("\\NRC\\XmlRemote\\Response\\{$class}", array(
			'body' => $this->_connectionResponse($class, $type)
		));
	}

	public function getClient($class, $type) {
		return $this->_instance("\\NRC\\XmlRemote\\Client\\{$class}", array(
	...
	protected function _starAcsSoapObject($xml) {
		...
		if (count($el) == 0)
			throw new \Exception('XmlRemoteTestCase - Malformed Soap Fixture');
